...
TimeScope automatically tracks which desktop apps (and, optionally, which website domains) you spend time on, then shows you a clean local dashboard — daily summaries, categories, goals, focus sessions, and hourly timelines. All the power of the big commercial time trackers — *but everything stays on your own machine.*
...
**ما هو TimeScope؟**
TimeScope هو **برنامج مجاني ومفتوح المصدر لتتبع الوقت تلقائياً**، مبني خصيصاً حول مبدأ "الخصوصية أولاً". يراقب تلقائياً التطبيق النشط على جهازك (اسم البرنامج، وقت البداية والنهاية، ومدة الاستخدام)، ويكتشف أوقات الخمول، ويعرض كل هذا بلوحة تحكم عصرية — بدون أي اتصال بالإنترنت.
...
**هل يغني TimeScope عن برامج تتبع الوقت التجارية المدفوعة؟**
نعم، إذا كنت تبحث عن برنامج تتبع وقت خصوصية-أولاً يعمل بدون اتصال إنترنت وبدون حساب سحابي، على ويندوز.
...
**What is TimeScope?**
TimeScope is a **free, open-source, privacy-first automatic time tracker** for Windows. It automatically records the active desktop application (name, start time, end time, duration), detects idle time, and presents everything in a modern local dashboard — with zero network calls by design.
...
**Is TimeScope a good alternative to paid commercial time trackers?**
Yes — if you want a privacy-first, local-only, open-source time tracker that works offline with no cloud account, TimeScope is built exactly for that.

**How is TimeScope different from commercial time trackers?**
TimeScope keeps 100% of your data local. Unlike closed-source cloud trackers, there is no server component at all — you can read every line of the tracking code yourself.
...
**TimeScope nedir?**
TimeScope, Windows için **ücretsiz, açık kaynaklı, gizlilik odaklı bir otomatik zaman takip uygulamasıdır**. Aktif masaüstü uygulamasını (ad, başlangıç/bitiş zamanı, süre) otomatik olarak kaydeder, boşta kalma süresini algılar ve her şeyi modern, tamamen yerel bir panelde sunar — hiçbir ağ isteği olmadan.
...
**TimeScope ücretli ticari zaman takip uygulamalarına iyi bir alternatif mi?**
Evet — çevrimdışı çalışan, bulut hesabı gerektirmeyen, gizlilik odaklı ve yerel bir zaman takip uygulaması arıyorsan, TimeScope tam olarak bunun için tasarlandı.
...
```
Build a production-ready desktop application for privacy-first automatic time tracking.
...
5. Dashboard
   ...
   Use a polished modern interface inspired by modern productivity apps and
...
like a privacy-first time tracker with its own identity, not a clone of any existing product.
```
...
> 🕒 TimeScope — a free, open-source, privacy-first time tracker for Windows. No cloud, no account, no telemetry. Every second of tracking data stays on your machine. 100% local. 100% yours. ⭐ github.com/EnesAhmet10000/timescope

**Medium (Reddit / LinkedIn):**
> TimeScope, a free and open-source automatic time tracking app for Windows that takes privacy seriously. It automatically tracks which apps (and optionally which website domains) you use, shows a clean local dashboard with categories, goals, and focus sessions — and never sends a single byte to the internet. No keystrokes, no passwords, no screenshots, ever. Everything lives in a local SQLite file you fully control. Would love feedback! 🕒

**Product Hunt tagline:**
> TimeScope — Track your time, not your privacy. A local-only, open-source time tracker for Windows.

**Hacker News title:**
> Show HN: TimeScope – an open-source, local-only automatic time tracker for Windows

### 🇸🇦 العربية

**قصير (تويتر/إكس):**
> 🕒 TimeScope — برنامج مجاني ومفتوح المصدر لتتبع الوقت، مصمم للخصوصية أولاً. بدون سحابة، بدون حساب، بدون تتبع خفي. كل بياناتك تبقى على جهازك فقط. ⭐ github.com/EnesAhmet10000/timescope

**متوسط (منشور تفصيلي):**
>  TimeScope، برنامج مجاني ومفتوح المصدر لتتبع الوقت على ويندوز، يتتبع وقتك تلقائياً ويأخذ الخصوصية على محمل الجد. يتتبع تلقائياً التطبيقات التي تستخدمها (والمواقع اختيارياً)، ويعرض لك لوحة تحكم أنيقة فيها تصنيفات وأهداف ووضع تركيز — وكل هذا بدون إرسال أي بيانات للإنترنت أبداً. لا كيبورد، لا كلمات مرور، لا لقطات شاشة. كل شي محفوظ محلياً بملف SQLite تتحكم فيه بالكامل. يسعدني أي رأي أو ملاحظة! 🕒

**عنوان لمنصة تقنية:**
> TimeScope — تتبّع وقتك، لا خصوصيتك. برنامج محلي ومفتوح المصدر لتتبع الوقت مخصص لويندوز.

### 🇹🇷 Türkçe

**Kısa (X/Twitter):**
> 🕒 TimeScope — Windows için ücretsiz, açık kaynaklı, gizlilik odaklı bir zaman takip uygulaması. Bulut yok, hesap yok, telemetri yok. Tüm izleme verileri kendi bilgisayarında kalır. %100 yerel, %100 senin. ⭐ github.com/EnesAhmet10000/timescope

**Orta (Reddit / LinkedIn):**
> TimeScope — gizliliği ciddiye alan, ücretsiz ve açık kaynaklı bir Windows otomatik zaman takip uygulaması. Kullandığın uygulamaları (ve isteğe bağlı olarak web sitesi alan adlarını) otomatik olarak izler, kategoriler, hedefler ve odak modu içeren temiz bir yerel panel sunar — ve asla internete tek bir veri göndermez. Tuş vuruşu yok, şifre yok, ekran görüntüsü yok. Her şey tamamen senin kontrolünde olan yerel bir SQLite dosyasında saklanır. Geri bildirimlerinizi bekliyorum! 🕒

**Tanıtım sloganı:**
> TimeScope — Zamanını takip et, gizliliğini değil. Windows için yerel, açık kaynaklı bir zaman takip uygulaması.

---

## 📄 License & Copyright / الرخصة والحقوق / Lisans ve Telif

**Copyright © 2026 EnesAhmet10000.** TimeScope is released under the **MIT License** — see the [LICENSE](LICENSE) file. You are free to use, copy, modify, and distribute it, provided the copyright notice is preserved.

TimeScope is an independent open-source project. It is **not affiliated with, endorsed by, or sponsored by any commercial time-tracking product or company**. All product names that may appear in historical documents are trademarks of their respective owners.

**حقوق النشر © 2026 EnesAhmet10000.** يصدر TimeScope تحت **رخصة MIT** — انظر ملف [LICENSE](LICENSE). يحق لك استخدامه ونسخه وتعديله وتوزيعه بحرية بشرط الحفاظ على إشعار الحقوق. TimeScope مشروع مفتوح المصدر مستقل وغير تابع لأي منتج أو شركة تجارية لتتبع الوقت.

**Telif Hakkı © 2026 EnesAhmet10000.** TimeScope, **MIT Lisansı** altında yayınlanır — [LICENSE](LICENSE) dosyasına bakın. Telif bildirimini korumak şartıyla özgürce kullanabilir, kopyalayabilir, değiştirebilir ve dağıtabilirsiniz. TimeScope bağımsız bir açık kaynak projesidir ve hiçbir ticari zaman takip ürünüyle bağlantılı değildir.

---

<p align="center">Made with ❤️ for privacy — TimeScope is free, open source, and always local-first.<br/>© 2026 EnesAhmet10000 · MIT License</p>
